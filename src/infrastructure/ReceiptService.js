...
import { ReceiptType, ResolutionType } from 'bitxor-sdk';
...
class ReceiptService {
  ...
  /**
	 * Gets a Token Resolution from searchCriteria.
	 * @param {object} resolutionStatementSearchCriteria Block Search Criteria.
	 * @returns {object} formatted token resolution data with pagination info.
	 */
  static searchTokenResolutionStatements = async resolutionStatementSearchCriteria => {
  	const searchTokenResolutionStatements = await http.createRepositoryFactory.createReceiptRepository()
  		.searchTokenResolutionStatements(resolutionStatementSearchCriteria)
  		.toPromise();

  	return {
  		...searchTokenResolutionStatements,
  		data: this.formatResolutionStatement(searchTokenResolutionStatements.data)
  	};
  }
   ...
   */
	static streamerAddressResolution = async searchCriteria => {
	 ...
	 /**
   * Gets a Token Resolution statement from streamer.
   * @param {object} searchCriteria - Search Criteria.
   * @returns {array} formatted statementTokenResolution[]
   */
	static streamerTokenResolution = async searchCriteria => {
		const streamerTokenResolution = await http.tokenResolutionStatementPaginationStreamer()
			.search(searchCriteria)
			.pipe(take(10), toArray())
			.toPromise();

		return this.formatResolutionStatement(streamerTokenResolution);
	}
   ...
   * @returns {object} Address Resolution | Token Resolution
   */
  static formatResolutionStatement = resolutionStatement => {
	  return resolutionStatement.map(statement => {
  		...
  		if (statement.resolutionType === ResolutionType.Token) {
  			return {
  				...statement,
  				resolutionType: Constants.ResolutionType[statement.resolutionType],
  				unresolved: statement.unresolved.toHex(),
  				tokenResolutionEntries: statement.resolutionEntries.map(resolutionEntry => resolutionEntry.resolved.toHex())
  			};
  		}
	  });
  }
