...
import { UInt64, TransactionGroup, Order, BlockOrderBy, BlockType, ReceiptType } from 'bitxor-sdk';
...
class BlockService {
  ...
  static getBlockReceiptsInfo = async height => {
  	...
  	const [address, token] = await Promise.all([
			  ...
			  ReceiptService.streamerTokenResolution(searchCriteria)
  	]);

  	return {
  		resolutionStatements: [...address, ...token]
  	};
  }
