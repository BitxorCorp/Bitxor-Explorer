...
import { Order, NamespaceId, UInt64, ReceiptType, Address } from 'bitxor-sdk';
...
class NamespaceService {
  ...
  /**
   * Get readable names for a set of tokens Returns friendly names for tokens.
   * @param {array} tokenIds - Array of token ids.
   * @returns {array} TokenNames.
   */
  static getTokensNames = async tokenIds => {
  	const tokenNames = await http.createRepositoryFactory.createNamespaceRepository()
  		.getTokensNames(tokenIds)
  		.toPromise();
  	const formattedTokenNames = tokenNames.map(tokenName => this.formatTokenName(tokenName));

  	return formattedTokenNames;
  }
  ...
  /**
   * Get linked tokenId from namespace Id.
   * @param {object} namespaceId - Namespace id
   * @returns {object} tokenId
   */
  static getLinkedTokenId = async namespaceId => {
  	const tokenId = await http.createRepositoryFactory.createNamespaceRepository()
  ...
  static getNamespaceInfo = async hexOrNamespace => {
  	...
  	if (namespace.aliasType === Constants.Message.TOKEN)
  		formattedNamespaceInfo.aliasToken = namespace.alias;

  	// End height disable click before expired.
  	formattedNamespaceInfo.expiredInBlock = helper.isInfinity(namespace)
  	...
  	if (currentHeight < formattedNamespaceInfo.endHeight) {
  		formattedNamespaceInfo.beforeEndHeight = helper.isInfinity(namespace)
		  ? Constants.Message.INFINITY
  	...
  	return {
  		...
  		data: namespaceInfos.data.map(namespace => {
  			...
  			return {
  				...namespace,
  				isExpired: isExpired,
  				approximateExpired: helper.isInfinity(namespace)
				  ? Constants.Message.INFINITY
  ...
  /**
   * Gets native namespaces name in from genesis transactions.
   * @returns {array} Namespaces
   */
  ...
  static formatAlias = alias => {
  	...
  	case 1: // Token id alias
  		return {
  			...alias,
  			aliasType: Constants.Message.TOKEN,
  			alias: alias?.tokenId.toHex()
  		};
  ...
  /**
   * Format token name to readable object.
   * @param {object} tokenName - tokenName DTO.
   * @returns {object} readable tokenName DTO.
   */
  static formatTokenName = tokenName => ({
  	...tokenName,
  	tokenId: tokenName.tokenId.toHex()
  })
