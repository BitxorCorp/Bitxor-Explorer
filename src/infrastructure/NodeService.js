...
class NodeService {
    /**
     * Get Storage Info from Bitxor SDK.
    ...
    /**
     * Get Node Peers from Bitxor SDK.
    ...
    static isNodeActive = async currentUrl => {
    	...
    	try {
    		await new bitxor.NodeHttp(currentUrl).getNodeHealth()
    			.toPromise();
    	} catch (e) {
    		status = false;
    	}
    ...
    static formatNodeInfo = nodeInfo => ({
    	...
    	address: bitxor.Address.createFromPublicKey(nodeInfo.publicKey, nodeInfo.networkIdentifier).plain(),
