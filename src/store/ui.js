...
import { NamespaceService, TokenService, AccountService } from '../infrastructure';
		...
		search: ({ dispatch, rootGetters }, _searchString) => {
			return new Promise(async (resolve, reject) => {
				if (null !== _searchString && '' !== _searchString) {
					...
					if (helper.isTokenOrNamespaceId(searchString)) {
						let result = void 0;

						try {
							result = await TokenService.getTokenInfo(searchString);
							if (result) {
								dispatch('openPage', {
									pageName: 'token',
									param: searchString
