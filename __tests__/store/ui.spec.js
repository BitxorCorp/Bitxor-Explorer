...
import { AccountService, TokenService, NamespaceService } from '../../src/infrastructure';
...
describe('store/ui', () => {
	describe('action search should', () => {
		...
		it('return token page with token Id', async () => {
			// Arrange:
			const tokenId = '3A8416DB2D53B6C8';

			const getTokenInfoStub = stub(TokenService, 'getTokenInfo');
			getTokenInfoStub.returns(Promise.resolve({}));

			// Act:
			await ui.actions.search({ dispatch, rootGetters }, tokenId);
		...
		it('throw error given Bxr address', async () => {
			// Arrange:
			const bxrAddress = 'TALICE546OTUY4YJTQCQZ4HSEP4UM5Y5KRG4JHD7';

			// Act + Assert:
			return expect(ui.actions.search({ dispatch, rootGetters }, bxrAddress))
