...
const TestHelper = {
	...
	/**
	 * Mock Block Info.
	 * @param {string} tokenId token id hex.
	 * @param {string} address account address.
	 * @param {number} startHeight start height.
	 * @param {number} duration duration.
	 * @returns {object} token info
	 */
	mockTokenInfo: (tokenId, address, startHeight, duration) => {
		return {
			tokenId: tokenId,
			divisibility: 0,
			address,
	 ...
	 * Mock token alias.
	 * @param {string} tokenId token id hex.
	 * @param {string} name alias name for token.
	 * @returns {object} token alias.
	 */
	mockTokenName: (tokenId, name) => {
		return {
			tokenId,
			names: [{
				name
	...
	mockTransaction: blockHeight => {
		return {
			...
			tokens: [
				new Token(new TokenId('7F2D26E89342D398'), UInt64.fromUint(5))
			]
		};
	},
	/**
	...
	mockInflationStatement: (amount, height) => {
		return {
			...
			tokenId: new TokenId('6BED913FA20223F8'),
			type: 20803,
			version: 1
		};
	}
