...
import { Token, UInt64, TokenId } from 'bitxor-sdk';
...
describe('Account Service', () => {
	...
	describe('getAccountList should', () => {
		 ...
		 */
		const setupMockAccounts = (tokens = [], isAccountAlias = false) => {
		...
		it('return accounts 0 balance given non network currency', async () => {
			// Arrange:
			const nonNativeCurrency = [new Token(new TokenId('19A9CFED0C0E752C'), UInt64.fromUint(1000))];
		...
		it('return accounts balance given network currency', async () => {
			// Arrange:
			const nativeCurrency = [new Token(new TokenId('6BED913FA20223F8'), UInt64.fromUint(1000))];
