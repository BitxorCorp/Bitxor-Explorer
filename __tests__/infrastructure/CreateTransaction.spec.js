...
describe('CreateTransaction should', () => {
	...
	it('return tokenMetadata', async () => {
		// Arrange:
		const mockTokenMetadata = {
			...mockTransactionMetadataDTO,
			value: metadataValue,
			targetAddress: randomAddress,
			targetTokenId: new TokenId('7F2D26E89342D398')
		};

		const getTokenAliasNamesStub = stub(Helper, 'getTokenAliasNames');
		getTokenAliasNamesStub.returns(Promise.resolve(['N/A']));

		// Act:
		const tokenMetadataObject = await CreateTransaction.tokenMetadata(mockTokenMetadata);

		// Assert:
		expect(tokenMetadataObject.transactionBody).toEqual({
			transactionType: mockTokenMetadata.type,
			scopedMetadataKey: mockTokenMetadata.scopedMetadataKey.toHex(),
			targetAddress: randomAddress.plain(),
			metadataValue: '00000020E6B189E5AD973B2070696E79696E3A2068C3A0 (Text: \x00\x00\x00 汉字; pinyin: hà)',
			valueSizeDelta: mockTokenMetadata.valueSizeDelta,
			targetTokenId: mockTokenMetadata.targetTokenId.toHex(),
			targetTokenAliasNames: ['N/A']
		});
	});
