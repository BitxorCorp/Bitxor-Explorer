...
describe('Bitxor Explorer Account Detail page', () => {
    beforeEach(() => {
    ...
    describe('Owned Tokens Card should', () => {
        it('load title', () => {
            cy.get('[data-cy="ownedTokensTitle"]').should('contain', 'Owned Tokens')
        })

        it('render table in card', () => {
            cy.renderTableInCard("ownedTokensTitle")
        })

        it('render correct table header.', () => {
            const items = ['Token ID', 'Amount', 'Alias Namespace']
            cy.renderHeaderInTable("ownedTokensTitle", items)
        })
    })
