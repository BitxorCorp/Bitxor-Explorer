				...
				stage('prepare variables') {
					steps {
						script {
							...
							build_image_full_name = "bitxorcorp/${build_image_repo}:${build_image_label}"
						}
					}
				}
				stage('print env') {
					...
					steps {
						script {
							run_docker_build_command = """
								python3 bitxor-mono/jenkins/bitxorcore/runDockerBuild.py \
									--compiler-configuration bitxor-mono/jenkins/bitxorcore/configurations/${COMPILER_CONFIGURATION}.yaml \
									--build-configuration bitxor-mono/jenkins/bitxorcore/configurations/${BUILD_CONFIGURATION}.yaml \
...
def get_public_version() {
	version_path = './bitxor-mono/jenkins/bitxorcore/server.version.yaml'
	data = readYaml(file: version_path)
	return data.version
}

def get_build_image_repo() {
	return is_public_build() ? 'bitxor-server' : 'bitxor-server-private'
}

def get_architecture_label() {
	data = readYaml(file: "./bitxor-mono/jenkins/bitxorcore/configurations/${COMPILER_CONFIGURATION}.yaml")
