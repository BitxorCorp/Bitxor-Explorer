		...
		stage('prepare') {
			stages {
				stage('prepare variables') {
					steps {
						script {
							dest_image_name = "bitxorcorp/bitxor-server-build-base:${OPERATING_SYSTEM}-${COMPILER_CONFIGURATION}"

							base_image_dockerfile_generator_command = """
								python3 ./jenkins/bitxorcore/baseImageDockerfileGenerator.py \
									--compiler-configuration jenkins/bitxorcore/configurations/${COMPILER_CONFIGURATION}.yaml \
									--operating-system ${OPERATING_SYSTEM} \
									--versions ./jenkins/bitxorcore/versions.properties \
							"""
						}
					}
				}
