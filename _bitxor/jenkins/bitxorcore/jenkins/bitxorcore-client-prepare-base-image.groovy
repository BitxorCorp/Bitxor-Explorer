pipeline {
	...
	stages {
		...
		stage('prepare Dockerfile') {
			steps {
				script {
					properties = readProperties(file: './jenkins/bitxorcore/versions.properties')
					version = properties[params.OPERATING_SYSTEM]

					dockerfile_template = "./jenkins/bitxorcore/templates/${params.OPERATING_SYSTEM.capitalize()}${params.IMAGE_TYPE.capitalize()}BaseImage.Dockerfile"
		...
		stage('build image') {
			steps {
				script {
					docker_image = docker.build "bitxorcorp/bitxor-server-${params.IMAGE_TYPE}-base:${params.OPERATING_SYSTEM}"
