...
INCLUDE_PRIORITIES_1LVL = {
	'"src': 100, '"mongo': 125, '"zeromq': 125, '"plugins': 150, '"bitxorcore': 200, '"bitxor': 200, '"tests': 500, '"test': 500
}
...
def check_include_depth(path_a, path_b):
	...
	# two elements (i.e. "bitxorcore/types.h") goes to bottom
...
class DepsConsole:
	...
	def include_filter(path):
		return 'bitxorcore' in path or 'plugins' in path or 'extensions' in path

	def deps_print(self, deps):
		for name, entries in sorted(deps.items()):
			for dependency_name, files in sorted(entries.items()):
				if self.args.deps_non_bitxorcore and dependency_name.startswith('bitxorcore'):
					continue
				if dependency_name != 'bitxorcore':
...
def parse_args():
	parser = argparse.ArgumentParser(description='bitxorcore lint checker')
	...
	deps.add_argument(
		'-n', '--non-bitxorcore', help='show only `.* -> non-bitxorcore` dependencies',
		action='store_true', dest='deps_non_bitxorcore'
	)
