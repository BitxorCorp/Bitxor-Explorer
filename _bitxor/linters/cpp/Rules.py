...
class DefaultRules:
	@staticmethod
	def check_bitxorcore_test(path_unified, splitted_path):
		...
		# but it has namespace bitxorcore::test
	...
	def namespace_check(ns_unified, full_path):
		...
		if ns_unified == 'bitxorcore:test:':
			return DefaultRules.check_bitxorcore_test(path_unified, splitted_path)

		if ns_unified == 'bitxorcore:mocks:':
			ns_unified = ':mocks:'
			if re.search(r':Mock', path_unified):
				return True

		# if there is no 'bitxorcore:test' in a path, but path looks like test
		if 'tests' in splitted_path and 'test' in splitted_path:
			return False
	...
	def validate_cross_includes(sorted_includes, path_elements):
		...
		for elem in sorted_includes:
			...
			if path_elements[1] == 'bitxorcore':
				if include_path_elements[1] == 'bitxorcore' and include_path_elements[2] != component:
					invalid.append(elem)

			if path_elements[1] == 'int':
				if include_path_elements[0] == 'tests' and include_path_elements[1] == 'bitxorcore':
					invalid.append(elem)
...
class PluginRules:
	...
	def namespace_check(ns_unified, full_path):
		...
		expected_unified = 'bitxorcore:plugins:'
		...
		if '.' not in next_part:
			expected_unified = 'bitxorcore:' + next_part + ':'
			if 'test' in path_elements:
				expected_unified = 'bitxorcore:test:'
				if 'mocks' in path_elements:
					expected_unified = 'bitxorcore:mocks:'
		elif path_elements[-1] in ['constants.h', 'types.h']:
			expected_unified = 'bitxorcore:'
...
class ExtensionRules:
	...
	def namespace_check(ns_unified, full_path):
		path_elements = split_path(full_path)
		expected_unified = 'bitxorcore:' + path_elements[1] + ':'
	...
	def validate_cross_includes(sorted_includes, path_elements):
		...
		for elem in sorted_includes:
			include_path_elements = elem.include[1:-1].split('/')
			if include_path_elements[0] in ['tests', 'bitxorcore']:
				continue
...
class ToolsRules:
	...
	def namespace_check(ns_unified, full_path):
		...
		path_unified = ':'.join(splitted_path)
		if ns_unified.startswith('bitxorcore:tools'):
			ns_unified = ns_unified[len('bitxorcore:'):]
