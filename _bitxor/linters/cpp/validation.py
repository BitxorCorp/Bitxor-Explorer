...
class TypoChecker(SimpleValidator):
	...
	def __init__(self):
		...
		self.errors = {
			...
			re.compile(r'[cC]ataputl'): 'bitxorcore not cataputl',
...
class BasicFunctionAliasValidator(SimpleValidator):
	...
	def check(self, line_number, line):
		if re.match(r'src.bitxorcore.functions.h', self.path) or re.match(r'src.bitxor.functions.h', self.path):
			return
...
class SpaceBraceValidator(SimpleValidator):
	...
	def __init__(self):
		...
		self.skip = [
			...
			# special for src/bitxorcore/utils/Logging.h
...
class UtilsSubdirValidator(SimpleValidator):
	...
	def reset(self, path, error_reporter):
		...
		if 'tests' in splitted and 'utils' in splitted:
			...
			# skip bitxorcore/utils
			dir_not_bitxorcore = splitted[idx - 1] != 'bitxorcore'  # bitxorcore-client
			dir_not_core = splitted[idx - 1] != 'core'  # sdk-cpp
			if idx > 0 and dir_not_bitxorcore and dir_not_core:
				self.has_utils = True
...
class MacroSemicolonValidator(SimpleValidator):
	...
	def __init__(self):
		...
		self.skip = [
			...
			re.compile(r'BITXORCORE_'),  # bitxorcore macros (LOG, THROW)
...
class ClosingBraceVerticalSpacingValidator(SimpleValidator):
	...
	def check(self, line_number, line):
		...
		# 2. another closing brace (with optional closing bitxors and/or semicolon and/or comma)
