...
class OptionsManager(BasicBuildManager):
	...
	def build_base_image_name(self):
		...
		return f'bitxorcorp/bitxor-server-build-base:{"-".join(name_parts)}'

	@property
	def prepare_base_image_name(self):
		return f'bitxorcorp/bitxor-server-test-base:{self.operating_system}'
...
def get_volume_mappings(ccache_path, conan_path, prepare_replacements):
	mappings = [
		(prepare_replacements['source_path'], '/bitxorcore-src'),
...
def prepare_docker_image(process_manager, container_id, prepare_replacements):
	...
	disposition_to_repository_map = {
		'tests': 'bitxor-server-test',
		'private': 'bitxor-server-private',
		'public': 'bitxor-server'
	}
	destination_repository = disposition_to_repository_map[build_disposition]

	destination_image_name = f'bitxorcorp/{destination_repository}:{destination_image_label}'
...
def main():
	...
	for folder_name in ['scripts', 'seed', 'resources']:
		environment_manager.copy_tree_with_symlinks(source_path / 'client/bitxorcore' / folder_name, folder_name)
