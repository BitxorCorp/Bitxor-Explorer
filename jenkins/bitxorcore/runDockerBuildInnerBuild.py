...
CONAN_BITXORCORP_REMOTE = 'https://bitxorcore.jfrog.io/artifactory/api/conan/bitxor-conan'
...
class LinuxEnvironment:
	...
	def prepare_conan(self, settings):
		...
		self.dispatch_subprocess(['conan', 'remote', 'add', '--force', 'bitxorcorp', CONAN_BITXORCORP_REMOTE])
...
class BuildManager(BasicBuildManager):
	...
	def cmake_settings(self, output_path):
		settings = [
			...
			('BITXORCORE_TEST_DB_URL', 'mongodb://db:27017'),
			('BITXORCORE_DOCKER_TESTS', 'ON'),
			('ENABLE_CODE_COVERAGE', 'ON' if self.enable_code_coverage else 'OFF'),
			('ARCHITECTURE_NAME', self.architecture)
		]

		if self.enable_diagnostics:
			settings.append(('ENABLE_BITXORCORE_DIAGNOSTICS', 'ON'))
		...
		if self.is_release:
			settings.append(('BITXORCORE_BUILD_RELEASE', 'ON'))
			settings.append(('ENABLE_TESTS', 'OFF'))

		if 'public' == self.build_disposition:
			settings.append(('BITXORCORE_BUILD_RELEASE_PUBLIC', 'ON'))
...
def main():
	parser = argparse.ArgumentParser(description='bitxorcore project build generator')
	...
	parser.add_argument('--source-path', help='path to the bitxorcore source code', required=True)
	parser.add_argument('--out-dir', help='output path to copy bitxorcore binaries', required=True)
